...
namespace GameVanilla.Game.Common
{
    ...
    public class GameBoard : MonoBehaviour
    {
        ...
        [HideInInspector] public int questionID = 0;
...
#pragma warning restore 649
        ...
        [HideInInspector] public bool onAnswerGiven;
        ...
        public void HandleInput()
        {
            ...
            if (drag && selectedTile != null)
            {
                ...
                if (hit.collider != null && hit.collider.gameObject != selectedTile)
                {
                    ...
                    {
                        drag = false;
                        CheckIfPerformMove(hit);
                    }
                  
                }
            }
        }
        // bool isMoveApplicable = false; // Commented out to fix CS0414 warning
        RaycastHit2D cacheraycasthit;

        public bool ApplicableMove(RaycastHit2D hit) {
        ...
        private void CheckIfPerformMove(RaycastHit2D hit)
        {

            if (!ApplicableMove(hit)) return;
                var combo = comboDetector.GetCombo(hit.collider.gameObject.GetComponent<Tile>(),
                    selectedTile.GetComponent<Tile>());
                if (combo != null && isQuestionAppeared )
                {
                    ...
                    tileA.GetComponent<Tile>().SetCandyPosition(idxB % level.width, idxB / level.width);
                    tileB.GetComponent<Tile>().SetCandyPosition(idxA % level.width, idxA / level.width);
                    ...
                    selectedTile = null;
                    print("PerformingHereDown");
                    PerformMove();
                }
                else if (possibleSwaps.Find(x => x.tileA == hit.collider.gameObject && x.tileB == selectedTile) !=
                         null ||
                         possibleSwaps.Find(x => x.tileB == hit.collider.gameObject && x.tileA == selectedTile) !=
                         null && isQuestionAppeared)
                {
                    var selectedTileCopy = selectedTile;
                    selectedTile.GetComponent<SpriteRenderer>().sortingOrder = 1;
                    currentlySwapping = true;
        ...
        private void InputNoMoves(RaycastHit2D hit)
        {
            print("No Moves");
            var selectedTileCopy = selectedTile;
        ...
        private bool HandleMatches(bool isPlayerMatch)
        {
            ...
            if (matches.Count > 0)
            {
                    StartCoroutine(OnMatchReady(isPlayerMatch, matches));
                    return true;
            }
            else
            {
        ...
        List<Match> cacheMatch;

      public void QuestionAnswered(bool answer) {

            if (answer)
            {
              //  PerformMove();
                CheckIfPerformMove(cacheraycasthit);
               // selectedTile = null;
            }
            else
            {
        ...
        {
            var selectedTileCopy = selectedTile;
            var hitTileCopy = cacheraycasthit.collider.gameObject;
            selectedTile.GetComponent<SpriteRenderer>().sortingOrder = 1;

            var selectedTilePos = tilePositions[tiles.FindIndex(x => x == selectedTile)];
            var hitTilePos = tilePositions[tiles.FindIndex(x => x == cacheraycasthit.collider.gameObject)];

            var tileA = cacheraycasthit.collider.gameObject;
            var tileB = selectedTile;
            if (!(tileA.GetComponent<Tile>().x != tileB.GetComponent<Tile>().x &&
                  tileA.GetComponent<Tile>().y != tileB.GetComponent<Tile>().y))
            {
                currentlySwapping = true;
                LeanTween.move(selectedTile, hitTilePos, 0.2f);
                LeanTween.move(cacheraycasthit.collider.gameObject, selectedTilePos, 0.2f).setOnComplete(() =>
                {
                    LeanTween.move(selectedTileCopy, selectedTilePos, 0.2f).setOnComplete(() =>
                    {
                        currentlySwapping = false;
                        selectedTileCopy.GetComponent<SpriteRenderer>().sortingOrder = 0;
                        selectedTileCopy.transform.rotation = Quaternion.identity;
                        cacheraycasthit.collider.gameObject.transform.rotation = Quaternion.identity;
                    });
                    LeanTween.move(hitTileCopy, hitTilePos, 0.2f);
                });
            }
        ...
        public IEnumerator OnMatchReady(bool isPlayerMatch, List<Match> matches)
        {
