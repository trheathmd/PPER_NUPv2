...
#pragma warning restore 649
        ...
        public void OnPauseButtonPressed()
        {
            if (currentPopups.Count == 0)
            {
                if (gameBoard.level.limitType == LimitType.Time)StopCoroutine(gameBoard.countdownCoroutine);
        ...
        public void EnableQuestionPopup()
        {
            if (currentPopups.Count == 0)
            {
                if(gameBoard.level.limitType ==  LimitType.Time)StopCoroutine(gameBoard.countdownCoroutine);

                OpenPopup<InGameSettingsPopup>("Popups/QuestionPopup");              
            }
            else
            {
                CloseCurrentPopup();
		...
		public void EnableBoosterMode(BuyBoosterButton button)
		{
			...
			switch (button.boosterType)
			{
				case BoosterType.Lollipop:
					ingameBoosterText.text = "Select a tile for the Syringe:";
					break;

				case BoosterType.Bomb:
					ingameBoosterText.text = "Select a tile for a band-aid bomb";
					break;
				...
				case BoosterType.ColorBomb:
					ingameBoosterText.text = "Select a tile for the Bandage:";
					break;
			}
		}
